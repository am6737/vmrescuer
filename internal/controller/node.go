...
type NodeWatcherInterface interface {
	...
	Migrate(ctx context.Context, vmi *virt.VirtualMachineInstance) error
}

// NodeWatcher 是您自定义节点操作的实现
type NodeWatcher struct {
	client.Client
}

func (n *NodeWatcher) Migrate(ctx context.Context, vmi *virt.VirtualMachineInstance) error {
...
func (n *NodeWatcher) GetPodByVMI(ctx context.Context, vmi *virt.VirtualMachineInstance) (*corev1.Pod, error) {
	...
	if len(candidatePods) == 0 {
		return nil, fmt.Errorf("no Pod found with matching label")
	}
